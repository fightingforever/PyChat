...
def sig_hdr(sig_num, frame):
    global ADDR, udp_client, tcp_server, video_server, history_fd
    
    if ADDR:
        try:
            udp_client.sendto(b'\x22\x22\x22', ADDR)
        except:
            pass
...
class TMsg(threading.Thread):
    def run(self):
        global udp_client, BUFSIZE, row, last_row, last_message, last_time, heart_beat_count, history_fd, is_you_leave, window_id, message_count, message_sound, is_yy_on
        ...
        try:
            while True:
                ...
                if data == b'\xFE\xDC\xBA':
                    ...
                    print_in_mid("Host left!", isInfo = True)
                    is_you_leave = True
                elif data == b'\x22\x22\x22':
                    print_in_mid("Host closed!", isInfo = True)
                elif data == "@yy":
                    if is_yy_on:
                    ...
                    if data[0] == '\0' and data[1] != '\0':
                        if is_you_leave:
                            print_in_mid("Host online!", isInfo = True)
                            is_you_leave = False
...
class TInput(threading.Thread):
    ...
    def run(self):
        global udp_client, ADDR, file_trans, instant_camera, row, last_row, message_flag, last_message, last_time, history_fd, is_you_leave, is_yy_on
            ...
            while True:
                ...
                if data == "":
                    ...
                    if data != "@history" and data != "@h" and is_you_leave:
                        print_in_mid("Host left, may cannot reply immediately", isInfo = True)
...
def main(argv):
    global HOST, ADDR, udp_client, tcp_server, video_server, file_trans, instant_camera, row, last_time, history_fd, window_id, message_sound
    ...
    while tMsg.isAlive() and tInput.isAlive() and file_trans.isAlive() and instant_video.isAlive() and instant_camera.isAlive() and heartBeat.isAlive() and message_notifier.isAlive():
        time.sleep(1)
